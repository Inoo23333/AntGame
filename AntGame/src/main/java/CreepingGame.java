...
public class CreepingGame {
    ...
    //添加一个静态的蚂蚁信息列表，保存所有蚂蚁的足迹信息
    public static List<OneAntRecord> arrayList = new ArrayList<>();
    ...
    public static void playOneGame(float incTime,int antNum,Ant[] ants,Stick stick)//只有这个方法需要对外可见
    {
        ...
        while(!isGameEnd){
            ...
            //打印
            printOneStep(antNum,ants);
            printOneStepToArrayList(antNum,ants);
    ...
    //+ 添加一个方法，每走一步把所有蚂蚁的位置和方向存入arrayList
    private  static void printOneStepToArrayList(int antNum,Ant[] ants){
        for(int i=0;i<antNum;i++){
            OneAntRecord oneAntRecord = new OneAntRecord(ants[i].getPosition(),ants[i].getDirection());
            arrayList.add(oneAntRecord);
        }
    }
